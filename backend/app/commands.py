...
from .models import Exceedance, Measurement, StandardVersion, Station


def register_commands(app):
    @app.cli.command("init-db")
    def init_db():
        """Create database tables and the default limit standard version."""
        from .services import standard_service

        db.create_all()
        standard_service.ensure_default_versions()
        click.echo("数据库表已创建, 默认限值标准版本已就绪")
    ...
    def stats():
        ...
        click.echo(
            "监测点 %d 个 / 监测数据 %d 条 / 超标记录 %d 条 / 标准版本 %d 个"
            % (
                Station.query.count(),
                Measurement.query.count(),
                Exceedance.query.count(),
                StandardVersion.query.count(),
            )
        )
