...
def test_value_below_limit_is_not_exceeded():
    result = exceedance_rules.evaluate("PM25", "daily", 60.0, 75.0)
...
def test_light_moderate_and_severe_grading():
    assert exceedance_rules.evaluate("PM25", "daily", 80.0, 75.0)["level"] == "light"
    assert exceedance_rules.evaluate("PM25", "daily", 120.0, 75.0)["level"] == "moderate"
    assert exceedance_rules.evaluate("PM25", "daily", 200.0, 75.0)["level"] == "severe"


def test_ratio_is_computed_against_limit():
    result = exceedance_rules.evaluate("NO2", "hourly", 250.0, 200.0)
...
def test_period_without_limit_is_recorded_but_not_flagged():
    result = exceedance_rules.evaluate("PM10", "hourly", 400.0, None)
...
def test_unknown_pollutant_raises():
    with pytest.raises(ValueError):
        exceedance_rules.evaluate("XX", "daily", 1.0, 75.0)


def test_summarize_counts_exceeded_items():
    results = [
        exceedance_rules.evaluate("PM25", "daily", 10.0, 75.0),
        exceedance_rules.evaluate("PM25", "daily", 90.0, 75.0),
    ]
